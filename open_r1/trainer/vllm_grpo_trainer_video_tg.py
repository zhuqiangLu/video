...
class Qwen2VLGRPOVLLMTrainer_Video_TG(Trainer):
    def __init__(
        self,
    ...
    def _prepare_inputs(
        ...
        prompts_text = [self.processing_class.apply_chat_template(self.make_conversation_video(example), tokenize=False, add_generation_prompt=True) for example in inputs]
        
        # image_inputs = [x["image_inputs"] for x in inputs]
        video_inputs = [x["video_inputs"] for x in inputs]
        fps_inputs = [x["fps_inputs"] for x in inputs]

        video_inputs = video_inputs[0]
        fps_inputs = fps_inputs[0]

        video_inputs, fps_inputs = self.post_process_video_inputs(video_inputs, fps_inputs, down=2.0)
        ...
        if self.args.use_vllm:
            ...
            all_videos = gather_object(video_inputs)
            # group into pairs

            all_videos = [v.cpu() if isinstance(v, torch.Tensor) else v for v in all_videos]
            ...
            process_slice = slice(
                self.accelerator.process_index * len(fpss),
                (self.accelerator.process_index + 1) * len(fpss),
            )
            ...
            # print(
            #         # 'input:', self.make_conversation_video(inputs[0]), 
            #         # 'prompt_text:', prompts_text,
            #         'prompt_ids', prompt_ids.shape,
            #         'completion', completion_ids.shape,
            #         'fps', fps_inputs,
            #     )
        ...
        if is_conversational(inputs[0]):    # 未包装进去
            completions = [
                [{"role": "assistant", "content": completion}]
                for completion in completions
            ]
            # print('is_conversational')
            # print(completions)
        ...
        # )
        rewards_per_func = torch.zeros(
            len(fpss), len(self.reward_funcs), device=device
        )
